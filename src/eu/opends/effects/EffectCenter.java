...
public class EffectCenter {
	...
	private WindshieldRainParticleEmitter windshieldRainParticleEmitter;
	...
	public EffectCenter(Simulator sim)
	{
		...
		// Vector3f sunLightDirection = new Vector3f(-0.2f, -0.9f, 0.2f); //
		// TODO make adjustable
		Vector3f sunLightDirection = new Vector3f(-0.5f, -0.5f, -0.5f);
		sun.setDirection(sunLightDirection.normalizeLocal());
		// sim.getSceneNode().addLight(sun);

		WeatherSettings weatherSettings = Simulator.getDrivingTask().getScenarioLoader().getWeatherSettings();
		snowingPercentage = Math.max(weatherSettings.getSnowingPercentage(), -1); // use
																	// -1
																					// to
																					// suppress
																					// construction
																					// of
																			// SnowParticleEmitter
		rainingPercentage = Math.max(weatherSettings.getRainingPercentage(), -1); // use
																					// -1
																					// to
																					// suppress
																					// construction
																					// of
																					// RainParticleEmitter
		fogPercentage = Math.max(weatherSettings.getFogPercentage(), -1); // use
																			// -1
																			// to
																			// suppress
																			// construction
		...
		if (isRaining) {
			// init rain
			ColorRGBA color = new ColorRGBA(0.9f, 0.9f, 0.9f, 0.99f);
			if (sim.getDrivingTask().getFileName().contains("Night"))
				color = new ColorRGBA(0.1f, 0.1f, 0.1f, 0.99f);

			rainParticleEmitter = new RainParticleEmitter(assetManager, rainingPercentage, color);
			sim.getSceneNode().attachChild(rainParticleEmitter);

			// Additional particle emitter for rainy effect on a windshield
			// windshieldRainParticleEmitter = new
			// WindshieldRainParticleEmitter(assetManager, rainingPercentage,
			// color);
			// sim.getSceneNode().attachChild(windshieldRainParticleEmitter);
		}
		...
		if (isFog || isBloom || isShadow) 
		{
			for (ViewPort viewPort : CameraFactory.getViewPortList()) 
			{
				...
				if (isBloom) {
					...
					// chassis.getMaterial().setColor("GlowColor",
					// ColorRGBA.Orange);
				...
				if (isShadow) {
					DirectionalLightShadowFilter dlsf = new DirectionalLightShadowFilter(assetManager, 1024, 3); // TODO
																													// make
																													// adjustable
					dlsf.setLight(sun);
					dlsf.setLambda(1f);
					dlsf.setShadowIntensity(100f); // TODO make adjustable
		 ...
		 * 
		 * if(isShadow) { ArrayList<ViewPort> viewPortList =
		 * CameraFactory.getViewPortList();
		 * 
		 * int shadowMapSize = 4096; if(viewPortList.size() > 1) shadowMapSize =
		 * 1024;
		 * 
		 * for(ViewPort viewPort : viewPortList) {
		 * DirectionalLightShadowRenderer dlsr = new
		 * DirectionalLightShadowRenderer(assetManager, shadowMapSize, 1);
		 * dlsr.setLight(sun);
		 * dlsr.setEdgeFilteringMode(EdgeFilteringMode.PCFPOISSON);
		 * viewPort.addProcessor(dlsr); }
		 * 
		 * shadowMapSize = 1024; DirectionalLightShadowRenderer dlsrBack = new
		 * DirectionalLightShadowRenderer(assetManager, shadowMapSize, 1);
		 * dlsrBack.setLight(sun);
		 * CameraFactory.getBackViewPort().addProcessor(dlsrBack);
		 * 
		 * DirectionalLightShadowRenderer dlsrLeft = new
		 * DirectionalLightShadowRenderer(assetManager, shadowMapSize, 1);
		 * dlsrLeft.setLight(sun);
		 * CameraFactory.getLeftBackViewPort().addProcessor(dlsrLeft);
		 * 
		 * DirectionalLightShadowRenderer dlsrRight = new
		 * DirectionalLightShadowRenderer(assetManager, shadowMapSize, 1);
	...
	public void update(float tpf) {
		...
		if (isRaining) {
			// Update rain location so that it moves with the car and a rainbox
			// attached to the car in particular
			try {
				Node rainNode = (Node) sim.getCar().getCarNode().getChild("rainNode");
				Spatial rainGeom = rainNode.getChild(0);
				rainParticleEmitter.setLocalTranslation(rainGeom.getWorldTranslation().clone());
			} catch (Exception e) {
				e.printStackTrace();
			...
			/*
			 * windshieldRainParticleEmitter.setLocalTranslation(sim.getCar().
			 * getPosition());
			 * windshieldRainParticleEmitter.setLocalRotation(sim.getCar().
			 * getRotation());
			 * 
			 * for (Particle particle :
			 * windshieldRainParticleEmitter.getParticles()) {
			 * particle.velocity.set(sim.getCar().getCarControl().
			 * getLinearVelocity()); particle.rotateSpeed = 1f; }
			 */

			if (rainingPercentageHasChanged) {
				rainParticleEmitter.setPercentage(rainingPercentage);
				// windshieldRainParticleEmitter.setPercentage(rainingPercentage);
