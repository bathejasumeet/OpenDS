...
public class SimulatorAnalogListener implements AnalogListener {
	...
	public SimulatorAnalogListener(Simulator simulator) {
		...
		steeringSensitivityFactor = Simulator.getSettingsLoader().getSetting(Setting.Joystick_steeringSensitivityFactor,
				1.0f);
		combinedPedalsSensitivityFactor = Simulator.getSettingsLoader()
				.getSetting(Setting.Joystick_combinedPedalsSensitivityFactor, 1.0f);
		acceleratorSensitivityFactor = Simulator.getSettingsLoader()
				.getSetting(Setting.Joystick_acceleratorSensitivityFactor, 1.0f);
		brakeSensitivityFactor = Simulator.getSettingsLoader().getSetting(Setting.Joystick_brakeSensitivityFactor,
				1.0f);
		clutchSensitivityFactor = Simulator.getSettingsLoader().getSetting(Setting.Joystick_clutchSensitivityFactor,
				1.0f);
	}
	...
	public void onAnalog(String binding, float value, float tpf) {
		...
		if (binding.equals("SteeringWheelLeft")) {
			float steeringValue = ((value * steeringSensitivityFactor) / tpf) * (float) maxTurningAngle;

			// System.out.println("left: " +
			// Math.round(steeringValue*100000)/1000f);
		...
		else if (binding.equals("AcceleratorUp") || binding.equals("AcceleratorDown")
				|| binding.equals("CombinedPedalsAccelerator")) {
