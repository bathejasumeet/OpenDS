...
public class TriggerCenter 
{
	...
	private static LinkedList<Spatial> roadObjectsTriggerList;
	...
	public static void addToRoadObjectsTriggerList(Spatial object){
		roadObjectsTriggerList.addLast(object);
	}
	...
	private void handleTrafficLightCollision(Map<String,Spatial> triggerList)
	{
		for (Entry<String, Spatial> trigger : triggerList.entrySet())
		{
			...
			sim.getCar().getCarNode().getChild("carNode_basicCar").getWorldBound().collideWith(triggerObject.getWorldBound(), results);
			if (results.size() > 0)
			//if(sim.getCar().getCarNode().getChild("carNode_basicCar").getWorldBound().intersects(triggerObject.getWorldBound()))
			{
				//to be added sim.getCar().getCarNode().getChild("carNode_basicCar").getWorldBound().intersects(triggerObject.getWorldBound())
	...
	private void handleRoadObjectsCollision(LinkedList<Spatial> triggerList)
	{
		...
		{	
			...
			if(trigger.getWorldTranslation().distance(car.getCarNode().getChild("carNode_basicCar").getWorldTranslation()) < 1000)
			{
				String triggerName = trigger.getName();
					
				// calculate collision of the car with a road object trigger

				Node triggerObject = (Node) sim.getTriggerNode().getChild(triggerName);
				
				// if car intersects with a trigger --> report trigger to HMI Center
			    CollisionResults results = new CollisionResults();
			    car.getCarNode().getChild("carNode_basicCar").getWorldBound().collideWith(triggerObject.getWorldBound(), results);
			    //car.getCarNode().getChild("carNode_basicCar").getWorldBound().intersects(triggerObject.getWorldBound());
			    if (results.size() > 0)
				{
