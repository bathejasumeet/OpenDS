...
public class DataWriter 
{
	...
	public void saveAnalyzerData() 
	{
		...
		if (curDate.getTime() - lastAnalyzerDataSave/*.getTime()*/ >= updateInterval) 
		{
			...
			write(
					...
					car.isEngineOn(), 
					((SteeringCar)car).getDistanceToCar(),
					((SteeringCar)car).getTimeToCollide(),
					((SteeringCar)car).getHeadwayTime(),
	...
	public void write(Date curDate, float x, float y, float z, float xRot,
			...
			boolean isEngineOn, float distanceAhead, float timeToCollide, float headWayTime, Quaternion OculusOrientation) 
	{
		DataUnit row = new DataUnit(curDate, x, y, z, xRot, yRot, zRot, wRot,
				linearSpeed, steeringWheelState, gasPedalState, brakePedalState,
				isEngineOn, distanceAhead, timeToCollide, headWayTime, OculusOrientation);
		this.write(row);
		...
		try {
			...
			for (DataUnit r : arrayDataList) {
				...
				sb.append(r.getDate().getTime() + ":" + r.getXpos() + ":"
						...
						+ r.getBrakePedalPos() + ":" + r.isEngineOn() + ":" + r.getDistanceAhead() + ":" + r.getTimeToCollide() + ":" + r.getHeadWay()  + occulusData + newLine
						);
			}
