 ...
 * @author Rafael Math
 */
public class TransmissionNotWorking {
	// for speed percentage computation
	private static final float speedAt100PercentMarker = 140f; // FIXME 140f;
																// --> 80f for
																// trucks
	...
	public static int directionChangeCtr = 1;
	// previous direction: forward = true, backward = false
	private boolean currentDirection = true;
	private Vector3f lastLocation = null;
	private float distanceChange = 0f;
	private boolean lastLocObtained = false;

	public TransmissionNotWorking(Car car) {
		this.car = car;
		...
		minRPM = 0; // scenarioLoader.getCarProperty(CarProperty.engine_minRPM,
					// SimulationDefaults.engine_minRPM);
		maxRPM = scenarioLoader.getCarProperty(CarProperty.engine_maxRPM, SimulationDefaults.engine_maxRPM);

		setGear(0, isAutomaticTransmission, false);
	}
	...
	public float getPowerPercentage(int gear, float currentSpeed) {
		...
		switch (gear) {
		case 1:
			powerPercentage = -2892.78f * x5 + 678.61f * x4 + 558.43f * x3 - 216.84f * x2 + 17.78f * x + 0.58f;
			break;
		case 2:
			powerPercentage = -4206.38f * x6 + 4837.65f * x5 - 1897.61f * x4 + 310.7f * x3 - 34.7f * x2 + 3.82f * x
					+ 0.43f;
			break;
		case 3:
			powerPercentage = -3.17f * x3 + 1.44f * x2 + 0.12f * x + 0.27f;
			break;
		case 4:
			powerPercentage = -1.27f * x3 + 0.64f * x2 + 0.24f * x + 0.20f;
			break;
		case 5:
			powerPercentage = -0.47f * x3 + 0.12f * x2 + 0.35f * x + 0.11f;
			break;
		case 6:
			powerPercentage = -0.27f * x3 + 0.24f * x2 + 0.18f * x + 0.01f;
			break;
		case -1:
			powerPercentage = -1154.84f * x4 + 134.09f * x3 + 21.63f * x2 + 0.5f * x + 0.57f;
			break;
		case 0:
			powerPercentage = 0;
			break;
	...
	public void updateRPM(float tpf) {
		// Set RPM to 0 when engine is off
		if (!car.engineOn) {
			currentRPM = 0f;
			return;
		}
		...
		if (gear == 0) {
			...
			currentRPM = Math.min(Math.abs(car.getCarControl().getCurrentVehicleSpeedKmHour() * selectedTransmission
					/ (wheelCircumference * 0.00006f)), maxRPM);
			// clutch
			float traction = car.getTraction();
			if (traction < 1.0f)
				currentRPM = (traction * currentRPM)
						+ ((1.0f - traction) * (0.2f + car.getAcceleratorPedalIntensity()) * maxRPM * 0.5f);
		...
		if (currentRPM < 300 && car.isEngineOn() && !isAutomaticTransmission) {
	...
	public void shiftDown(boolean isAutomatic) {
		...
		// if automatic transmission --> do not shift down to N and R
		// automatically
		if (isAutomatic)
	...
	public void performAcceleration(float pAccel) {
		...
		// Change engine sound
		if (!car.engineOn) {
			AudioCenter.stopSound("engineIdle");
			AudioCenter.stopSound("idle_low");
		} else if (car.getClutchPedalIntensity() > 0.3 && car.getClutchPedalIntensity() < 0.7 && car.engineOn
				&& gear != 0 && currentEngineSpeed < 1200) {
			AudioCenter.playSound("idle_low");
			AudioCenter.stopSound("engineIdle");
		} else if (car.engineOn) {
			AudioCenter.playSound("engineIdle");
			AudioCenter.stopSound("idle_low");
		}
	...
	public void setGear(int gear, boolean isAutomatic, boolean rememberGear) {
		...
		// if manual transmission check if clutch is properly pressed before
		// shifting the gear.
		if ((!isAutomaticTransmission && car.getClutchPedalIntensity() > 0.8) || isAutomaticTransmission) {
			switch (this.gear) {
			case -1:
				selectedTransmission = reverseGear;
				break;
			case 0:
				selectedTransmission = neutralGear;
				break;
			default:
				selectedTransmission = forwardGears[this.gear - 1];
				break;
			}
		}
		if (gear == -1 && currentDirection) {
			currentDirection = false;
			directionChanged = true;
		} else if (gear > 0 && !currentDirection) {
			currentDirection = true;
			directionChanged = true;
		}
		if (car.getCarNode() != null && !lastLocObtained) {
			lastLocation = car.getCarNode().getLocalTranslation().clone();
			lastLocObtained = true;
		}

		if (car.getCarNode() != null && directionChanged) {
			Vector3f currentLocation = car.getCarNode().getLocalTranslation();
			distanceChange = (float) Math.sqrt(
					Math.pow(currentLocation.x - lastLocation.x, 2) + Math.pow(currentLocation.y - lastLocation.y, 2)
							+ Math.pow(currentLocation.z - lastLocation.z, 2));
			if (distanceChange > 1f) {
