...
public class ScenarioLoader 
{
	...
	public ScenarioLoader(DrivingTaskDataQuery dtData, SimulationBasics sim, DrivingTask drivingTask, boolean oldLogic) 
	{
		this.dtData = dtData;
		this.sim = sim;
		...
		processSceneCar();
		extractTraffic(oldLogic);
		extractCameraFlight();
		extractConversionMatrices();
		extractFrictionMap();
		
		if(sim instanceof DriveAnalyzer | sim instanceof Simulator)
			extractIdealLine();
	...
	public RainSettings getRainSettings(){
		
		Boolean status = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:enabled", Boolean.class);
		if (status == null){
			status = false;
		}
		
		Float scaleX = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:scaleX", Float.class);
		if (scaleX == null){
			scaleX = 1f;
		}
		
		Float scaleY = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:scaleY", Float.class);
		if (scaleY == null){
			scaleY = 1f;
		}
		
		String mode = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:mode", String.class);
		if (mode == null){
			mode = "front";
		}
		
		String path = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:texturesPath", String.class);
		if (path == null){
			path = "none";
		}
		
		Integer fps = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:updateSpeed", Integer.class);
		if (fps == null){
			fps = 0;
		}
		
		String rainSurfaceName = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:nameRainSurface", String.class);
		if (rainSurfaceName == null){
			rainSurfaceName = "none";
		}
		
		String frontSurfaceName = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:nameFront", String.class);
		if (frontSurfaceName == null){
			frontSurfaceName = "none";
		}
		
		String SideLSurfaceName = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:nameLeft", String.class);
		if (SideLSurfaceName == null){
			SideLSurfaceName = "none";
		}
		
		String SideRSurfaceName = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:nameRight", String.class);
		if (SideRSurfaceName == null){
			SideRSurfaceName = "none";
		}
		
		String otherSurfaceName = dtData.getValue(Layer.SCENARIO,
				"/scenario:scenario/scenario:environment/scenario:realisticRain/scenario:nameOther", String.class);
		if (otherSurfaceName == null){
			otherSurfaceName = "none";
		}
		
		return new RainSettings(status, scaleX, scaleY, mode, fps, path, rainSurfaceName, frontSurfaceName, SideLSurfaceName, SideRSurfaceName, otherSurfaceName);
	}
	...
	{
		Boolean useWaterFilter = dtData.getValue(Layer.SCENARIO, 
				"/scenario:scenario/scenario:environment/scenario:useWaterFilter", Boolean.class);
		
		if(useWaterFilter == null)
			useWaterFilter = false;
		
		return useWaterFilter;
	}
		...
		try {
			...
			for (int k = 1; k <= oDvehicleNodes.getLength(); k++) 
			{
				...
				OpenDRIVECarData openDRIVECarData = new OpenDRIVECarData(name, mass, acceleration, decelerationBrake, 
						...
						maxSpeed, startRoadID, startLane, startS);
	...
	private void extractTraffic(boolean oldLogic)
	{
		try {
			NodeList vehicleNodes = (NodeList) dtData.xPathQuery(Layer.SCENARIO, 
					"/scenario:scenario/scenario:traffic/scenario:vehicle", XPathConstants.NODESET);
			...
			for (int k = 1; k <= vehicleNodes.getLength(); k++) 
			{
				...
				if (oldLogic) {
					trafficCarData = new TrafficCarData(name, mass, acceleration, decelerationBrake, 
		...
		try {
			...
			for (int k = 1; k <= pedestrianNodes.getLength(); k++) 
			{
				...
				if (oldLogic) {
				pedestrianData = new PedestrianData(name, enabled, mass, animationStand, animationWalk, 
	...
	public FollowBoxSettings getAutoPilotFollowBoxSettings(boolean oldSettings) 
	{
		if (oldSettings) {
			...
			return getAutoPilotFollowBoxSettings();
		}
	}
