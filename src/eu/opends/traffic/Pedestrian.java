...
public class Pedestrian implements AnimationListener, TrafficObject
{
    ...
    private void init()
    {
		...
		followBox = new FollowBox(sim, this, pedestrianData.getFollowBoxSettings(), true, true);
    	...
    	if(initialized)
    	{
			if(!sim.isPause())
			{
		        ...
		        else 
		        {
		            ...
		            {
		            	if (!animationStandCommand.equals(animationController.getAnimationName()))
		            		//animationController.animate(animationName, speedMultiplier, blendTime, loopCount);
		            		animationController.animate(animationStandCommand, 1f, 1f, 0);
		            }
